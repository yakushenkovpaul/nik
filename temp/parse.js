...
let parse_product = (product_url) => {
	...
	https.get(product_url, { headers }, (res) => {
		...
		console.log('get [' + jsonStr.length + '] [' + product_url + ']');
		...
		if(jsonStr.length > 0)
		{
			...
			if(uniq_key > 0)
			{
				if(!uniq_keys.includes(uniq_key)) 
				{
					...
					console.log('added product_url [' + data.url + ']');
	...
	https.get(page_url, { headers }, (res) => {
		...
		res.on('end', () => {
			...
			urls.each((index, element) => {
				setTimeout(() => {
					parse_product($(element).attr('href'))
					current++;
					if(current == total)
					{
						if ($('i[aria-label="chevron_right"]').length > 0) {
							if(page < 13)
							{
								parse('https://www.99.co/id/jual/rumah/bali?hlmn=' + page);
							}
						}
					}
				}, 1500 * index);
...
parse('https://www.99.co/id/jual/rumah/bali?hlmn=1')