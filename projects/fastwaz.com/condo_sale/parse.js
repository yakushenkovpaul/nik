...
const parse_product = (product_url, callback) => {
	...
	https.get(product_url, { headers }, (res) => {
		...
		getTopDetails($, data);
		data.sale_price = $('div.gallery-unit-sale-price__price').contents().not('.group-price').text().trim();
		//data.reduce_price = $('.group-price').find('.gallery-unit-sale-price__price-reduce').text().trim();
